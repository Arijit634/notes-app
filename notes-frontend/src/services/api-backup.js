...
export const profileAPI = {
  ...
  verifyTwoFactor: async (code) => {
    const response = await apiClient.post(API_ENDPOINTS.PROFILE.VERIFY_2FA, { code });
    return response.data;
  },

  disableTwoFactor: async (code) => {
    const response = await apiClient.post(API_ENDPOINTS.PROFILE.DISABLE_2FA, { code });
    return response.data;
  },
};

// Export everything
export { apiClient, apiClient as apiService, profileAPI, tokenUtils };
